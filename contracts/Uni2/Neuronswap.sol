...
// neuron 0.3% swap fee, 0.03% payback to neuron reward
library NeuronHelper {
    address public constant router = 0x135F4B1A18Fc6d73b1DAaaFC83D5983e9D90a82E;
    address public constant WKLAY = 0xfd844c2fcA5e595004b17615f891620d1cB9bBB2;
    address public constant factory = 0xe334e8c7073e9AaAE3cab998EECcA33F56df6621;
    INeuronswap public constant CPMMRouter = INeuronswap(router);
    uint32 public constant fee = 3000;
}

interface INeuronswap {
...
contract Neuronswap is Uni2 {
        ...
        returns (uint256[] memory amounts)
    {
        ...
        amounts = NeuronHelper.CPMMRouter.getAmountsOut(amountIn, uni2Path);
    }
        ...
        returns (uint256 output)
    {
        ...
        address[] memory uni2Path = _makeRoutePath(path);
        uint256[] memory amounts = NeuronHelper.CPMMRouter.getAmountsOut(msg.value, uni2Path);
        output = amounts[amounts.length - 1];
        require(output >= amountOutMin, "UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT");

        IWKLAY(NeuronHelper.WKLAY).deposit{ value: amounts[0] }();

        IERC20(uni2Path[0]).uniTransfer(
            IUniswapV2Factory(NeuronHelper.factory).getPair(uni2Path[0], uni2Path[1]),
            amounts[0]
        );
    ...
    function swapExactKct(
        ...
        address[] memory uni2Path = _makeRoutePath(path);
        uint256[] memory amounts = NeuronHelper.CPMMRouter.getAmountsOut(amountIn, uni2Path);
        output = amounts[amounts.length - 1];
        require(output >= amountOutMin, "UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT");
    ...
    function poolInfos() external view override returns (IUni2Viewer[] memory, uint256) {
        IUniswapV2Factory WFactory = IUniswapV2Factory(NeuronHelper.factory);
    ...
    function tokenInfos() external view override returns (ITokenViewer[] memory) {
        uint256 poolNum = IUniswapV2Factory(NeuronHelper.factory).allPairsLength();
        uint256 tokenNum;
        ...
        for (uint256 i; i < poolNum; i++) {
            IUniswapV2Pair pool = IUniswapV2Pair(IUniswapV2Factory(NeuronHelper.factory).allPairs(i));
