...
// definix 0.25% swap fee
library DefinixHelper {
    address public constant router = 0x4E61743278Ed45975e3038BEDcaA537816b66b5B;
    address public constant WKLAY = 0x5819b6af194A78511c79C85Ea68D2377a7e9335f;
    address public constant herodotus = 0x2aFc6d4bAbCDfba8fAD56f7D25C9F930E9c093Ad;
    address public constant factory = 0xdEe3df2560BCEb55d3d7EF12F76DCb01785E6b29;
    IDefinixRouter public constant CPMMRouter = IDefinixRouter(router);
...
interface IDefinixRouter is IDefinixRouter01 {
...
contract Definix is Uni2 {
        ...
        returns (uint256[] memory amounts)
    {
        ...
        amounts = DefinixHelper.CPMMRouter.getAmountsOut(amountIn, uni2Path);
    }
        ...
        returns (uint256 output)
    {
        ...
        address[] memory uni2Path = _makeRoutePath(path);
        uint256[] memory amounts = DefinixHelper.CPMMRouter.getAmountsOut(msg.value, uni2Path);
        output = amounts[amounts.length - 1];
        require(output >= amountOutMin, "UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT");

        IWKLAY(DefinixHelper.WKLAY).deposit{ value: amounts[0] }();

        IERC20(uni2Path[0]).uniTransfer(
            IUniswapV2Factory(DefinixHelper.factory).getPair(uni2Path[0], uni2Path[1]),
            amounts[0]
        );
    ...
    function swapExactKct(
        ...
        address[] memory uni2Path = _makeRoutePath(path);
        uint256[] memory amounts = DefinixHelper.CPMMRouter.getAmountsOut(amountIn, uni2Path);
        output = amounts[amounts.length - 1];
        require(output >= amountOutMin, "UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT");
    ...
    function pools() external view override returns (address[] memory poolAddrs) {
        uint256 rawPoolNum = IDefinixHerodotus(DefinixHelper.herodotus).poolLength();
        uint256 poolNum;
        for (uint256 i; i < rawPoolNum; i++) {
            (address pool, , , ) = IDefinixHerodotus(DefinixHelper.herodotus).poolInfo(i);
            if (
                keccak256(abi.encodePacked(IUniswapV2Pair(pool).symbol())) == keccak256(abi.encodePacked("DEFINIX-LP"))
            ) {
                poolNum = poolNum + 1;
            }
        }
    ...
    function poolInfos() external view override returns (IUni2Viewer[] memory, uint256) {
        ...
        uint256 idx;
        for (uint256 i; idx < poolNum; i++) {
            (address pool, , , ) = IDefinixHerodotus(DefinixHelper.herodotus).poolInfo(i);
            if (
                keccak256(abi.encodePacked(IUniswapV2Pair(pool).symbol())) == keccak256(abi.encodePacked("DEFINIX-LP"))
            ) {
                pools[idx] = this.poolInfo(pool);
                idx = idx + 1;
            }
        }
    ...
    function tokenInfos() external view override returns (ITokenViewer[] memory) {
        ...
        uint256 poolIdx;
        for (uint256 i; poolIdx < poolNum; i++) {
            (address Pool, , , ) = IDefinixHerodotus(DefinixHelper.herodotus).poolInfo(i);
            if (
                keccak256(abi.encodePacked(IUniswapV2Pair(Pool).symbol())) == keccak256(abi.encodePacked("DEFINIX-LP"))
            ) {
                ...
                if (token0 != address(0)) {
                    ...
                    if (flag == 0) {
                        tokenInfoDicts.key[poolIdx * 2] = token0;
                        tokenInfoDicts.flag[poolIdx * 2] = 1;
                        tokenInfoDicts.tokenInfo[poolIdx * 2] = this.tokenInfo(token0);
                        tokenNum = tokenNum + 1;
                    }
                }
                if (token1 != address(0)) {
                    ...
                    if (flag == 0) {
                        tokenInfoDicts.key[poolIdx * 2 + 1] = token1;
                        tokenInfoDicts.flag[poolIdx * 2 + 1] = 1;
                        tokenInfoDicts.tokenInfo[poolIdx * 2 + 1] = this.tokenInfo(token1);
                        tokenNum = tokenNum + 1;
                    }
                }
                poolIdx = poolIdx + 1;
